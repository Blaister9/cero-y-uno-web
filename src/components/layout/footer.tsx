...
export function Footer() {
  ...
  return (
    <footer className="border-t border-white/10 bg-ink py-10">
      <Container className="grid gap-8 md:grid-cols-2 lg:grid-cols-[1.1fr_0.65fr_0.75fr_0.8fr]">
        <div>
        ...
        <div>
          <p className="text-sm font-semibold text-porcelain">Navegación</p>
          <ul className="mt-3 grid gap-2 text-sm text-steel">
            {siteConfig.navItems.map((item) => (
              <li key={item.href}>
                <Link className="transition hover:text-porcelain" href={item.href}>
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <a
            className="mt-4 inline-flex w-fit items-center gap-2 text-xs text-steel/85 transition hover:text-porcelain"
            href={siteConfig.productionUrl}
          >
            URL temporal productiva
